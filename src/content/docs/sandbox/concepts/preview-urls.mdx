...
- Port 8080 with auto-generated token: `https://8080-abc123-abc123random4567.yourdomain.com`
- Port 3000 with custom token: `https://3000-abc123-my-stable-token.yourdomain.com`

**Local development**: `http://localhost:8787/...`

## URL Stability

By default, preview URLs use randomly generated 16-character tokens. If you unexpose and re-expose a port, a new token is generated and the URL changes.

For stable URLs across container restarts and deployments, use custom tokens:

```typescript
// Random token - URL changes on re-expose
const { url } = await sandbox.exposePort(8080, { hostname });
// https://8080-abc123-abc123random4567.yourdomain.com

// Custom token - URL stays the same
const { url } = await sandbox.exposePort(8080, {
  hostname,
  token: 'my-stable-token'
});
// https://8080-abc123-my-stable-token.yourdomain.com
```

Custom tokens must be 1-63 characters and contain only lowercase letters, numbers, hyphens, and underscores. This ensures URL and DNS compatibility.
...
- **Token-based access** - Each exposed port gets a unique token in the URL (for example, `https://8080-sandbox-abc123random4567.yourdomain.com`)
- **HTTPS in production** - All traffic is encrypted with automatic TLS
- **Unpredictable URLs** - By default, tokens are randomly generated (16 characters) and difficult to guess. Custom tokens provide stable URLs but are more predictable.
