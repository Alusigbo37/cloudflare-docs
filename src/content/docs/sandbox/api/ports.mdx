...
- `options`:
  ...
  - `token` - Custom token for the preview URL (optional). If not provided, a random 16-character token is generated automatically. Must be 1-63 characters containing only lowercase letters, numbers, hyphens, and underscores. Use custom tokens to maintain stable preview URLs across container restarts and deployments.
...
// https://8000-abc123-abc123random4567.yourdomain.com (random token)
...
const stableApi = await sandbox.exposePort(8080, {
  hostname,
  token: 'my-api-token'
});
// https://8080-abc123-my-api-token.yourdomain.com (always the same URL)
```
</TypeScriptExample>
