...
By default, preview URLs use random 16-character tokens that change when you re-expose ports. For stable URLs across container restarts and deployments, use custom tokens:
...
// https://8080-abc123-my-api-server.yourdomain.com

// Without token - URL changes on each re-expose
const { exposedAt: randomUrl } = await sandbox.exposePort(8081, { hostname });
// https://8081-abc123-abc123random4567.yourdomain.com
```
</TypeScriptExample>

Custom tokens must be 1-63 characters containing only lowercase letters, numbers, hyphens, and underscores.

**When to use custom tokens:**
- Sharing URLs with users or documentation
- Webhooks or API callbacks that need stable endpoints
- Production deployments where URL consistency matters

**When to use random tokens:**
...
- Port 8080 with random token: `https://8080-abc123-abc123random4567.yourdomain.com`
- Port 5173 with custom token: `https://5173-abc123-my-frontend.yourdomain.com`
